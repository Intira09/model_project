...
Original file is located at
    https://colab.research.google.com/drive/1_-E6yLt2UT_vj6AMKVnM6K0RHD6wYUkT
...
"""# สร้าง list คำที่มักาะกดผิดแต่โมเดลไม่เจอ"""
...
common_misspellings = [
  ...
  {"wrong": "เทคโนโลย", "right": "เทคโนโลยี"},
...
"""# สะกดคำ+ใจความ ได้ 88/100

ปรับโดยเพิ่ม list ของคำที่มักสะกดผิด และตรวจการสะกดคำก่อนค่อยทำ tokens

https://silpakorn-my.sharepoint.com/:x:/g/personal/singtong_c2_su_ac_th/EX7WFpWtb2pKiML35Vh8kggB0QarYjMjELbbs6lmJDy3dA?e=dgBGDs
"""
...
def spellcheck_before_tokenize(text):
    words = re.findall(r'[ก-๙]+', text)  # ✅ ดึงเฉพาะคำไทยดิบ
...
def evaluate_text(text):
    ...
    if error_counts["split"]:
        ...
        reasons.append("พบการแยกคำผิด: " + "; ".join(details))
    if error_counts["spelling"]:
        error_words = [f"{e['word']} (แนะนำ: {', '.join(e.get('suggestions', []))})" for e in all_spelling_errors]
...
# --- โหลดไฟล์คำตอบ ---
# โหลด dataset เดิม
df = pd.read_csv("/content/drive/MyDrive/new100_dataset.csv")
...
df.to_csv("/content/sample_data/new14_score_s4.csv", index=False, encoding="utf-8-sig")
